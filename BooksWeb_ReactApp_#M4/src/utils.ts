...
function searchBooks(books:any,searchBy:string,searchText:string){
    if(searchBy===null || searchText===''){
        return books;
    }
    switch(searchBy){
        case "id":let bk= books?.find((b:any)=>b.id===Number(searchText));
                    return [bk];
        case "title":return books?.filter((b:any)=>b.title.toLowerCase().indexOf(searchText.toLowerCase())!==-1);
        case "author":return books?.filter((b:any)=>b.author.toLowerCase().indexOf(searchText.toLowerCase())!==-1);
        case "rating":return books?.filter((b:any)=>b.rating>=searchText);
        case "price":let [priceLower,priceHigher]=searchText.split('-');
                    return books?.filter((b:any)=>b.price>=priceLower&&b.price<=priceHigher);
    }
}
export {searchBooks}