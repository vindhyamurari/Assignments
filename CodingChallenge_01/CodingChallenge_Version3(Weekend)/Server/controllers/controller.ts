...
//GET All Directors 
async function getAllDirectors(request:any,response:any) {
    try{
        let allDirectors=await directors.find();
        return response.status(200).send({success:true,message:"Fetched All Directors",allDirectors})
    }
    catch(err){
...
//GET All Directors 
async function getAllMovies(request:any,response:any) {
    try{
        let allMovies=await movies.find();
        return response.status(200).send({success:true,message:"Fetched All Movies",allMovies})
    }
    catch(err){
        return response.status(404).send({success:false,message:'Something Went Wrong Try Again'})
    }
    ...
    try{
        try{
            ...
            if(director){
                ...
                return response.status(200).send({success:true,message:'Director details Updated',allMoviesOfGivenDirector,director})
            }
            else{
                return response.status(404).send({success:false,message:'Director Not Found'})
            }
...
export {getAllDirectors,addNewDirector,updateDirectorDetails,getDirectorInfo, deleteOneMovie,addNewMovie,getMovieInfo,getAllMovies}