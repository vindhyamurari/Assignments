export async function displayBookList(searchPattern,keyWord){
    ...
    //
    if(searchPattern=="rating"){
      uri+=`?${searchPattern}_lte=${keyWord}`;
      console.log("rating",uri);
    }
    else if(searchPattern=="price"){
      let priceValues=keyWord.split('-');
      uri+=`?${searchPattern}_gte=${priceValues[0]}&${searchPattern}_lte=${priceValues[1]}`;
      console.log("price",uri);
    }
    else if(searchPattern=="id"){
