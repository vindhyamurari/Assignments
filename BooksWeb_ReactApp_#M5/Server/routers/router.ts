...
import {registerUser,loginUser,isAuthorized,generateAndSendOTP,recieveAndCheckOTP} from '../controllers/userController'
...
router.post('/users/sendOTP',(request,response)=>{
    generateAndSendOTP(request,response);
})

router.post('/users/verifyOTP',(request,response)=>{
    recieveAndCheckOTP(request,response);
})
