...
describe('testing api using supertest', () => {
    ...
    describe('testing for authenticated users', () => {
        ...
        it('should add a movie successfully from an authentic user', () => __awaiter(void 0, void 0, void 0, function* () {
            ...
            // expect(response.body.movie._id).not.toBeNull();
            // const movieResult=await request(app).get(`/api/movies/${response.body.movie._id}`);
            // expect(movieResult.status).toBe(200); 
            // //expect(movieResult.body.name).toStrictEqual('Time');
            // expect(movieResult.body.name).toStrictEqual(response.body.movie.name);
        }));
    });
});
